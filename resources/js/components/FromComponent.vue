    ...
    <div class="section__inner">
      ...
      <div class="from-lists">
        ...
        <table class="from-lists__table">
          ...
          <template v-for="(from, key) in froms">
            <tr class="from-lists__table-row" :key="key">
              <td>{{ from.l_name}} {{ from.f_name }}</td>
              <td>{{ from.postal_code }}</td>
              <td>{{ from.prefecture.name }}{{ from.city.name }}{{ from.address_etc }}</td>
              <td><button class="edit" @click="setUpdate(from.id)">編集</button></td>
              <td><button class="delete" @click="del(from.id)">削除</button></td>
            </tr>
          </template>
...
export default {
  ...
  methods: {
    ...
    create: function(){
      ...
      // 登録処理
      axios.post('/from/create', this.postData).then(res => {
        console.log(res);
        // 一覧再描画
        this.setList();
        // フォームクリア
    ...
    setUpdate: function(id){
      ...
      this.updateId = id;
      // フォームセット
      axios.get('/from/set_update/' + id).then(res => {
        console.log(res);
    ...
    update: function(){
      ...
      // 更新処理
      axios.post('/from/update/' + this.updateId, this.postData).then(res => {
        console.log(res);
        // 一覧再描画
        this.setList();
        // フォームクリア
    ...
    del: function(id){
      ...
      axios.delete('/from/delete/' + id).then(res => {
        console.log(res);
        // 一覧再描画
        this.setList();
        // フォームクリア
